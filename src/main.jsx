...
function Dashboard({daysLeft,setActive}){return <section className="hero page heroCinema"><motion.div initial={{opacity:0,y:25}} animate={{opacity:1,y:0}} className="heroInner heroPanel"><div className="eyebrow"><Sparkles size={16}/> TUSCANY AFTERGLOW • LUXURY RESORT INTELLIGENCE</div><h1>OPERATION ALBATROS 2026</h1><p className="lead">16 reizigers. 24 dagen. 1 camping. Geen garanties.</p><p className="heroSub">De vakantie begint hier — met golden hour licht, zwembadvibes en een beetje geheimzinnige Toscaanse magie.</p><div className="cta"><button onClick={()=>setActive('reizigers')}>Reizigers</button><button className="ghost" onClick={()=>setActive('route')}>Bekijk route & hotels</button></div></motion.div><div className="storyGrid"><article className="storyCard"><h3>Mission briefing</h3><p>Een cinematic opening met Toscaanse zonsondergang, luxe resort sfeer en het gevoel dat de vakantie officieel begint.</p></article><article className="storyCard"><h3>Camping experience</h3><p>Zwembad, strand, Aperol-achtige zomerlichtjes en een premium vakantiegevoel dat elke dag uitnodigt.</p></article><article className="storyCard"><h3>Golden hour route</h3><p>Van Duitsland naar Italië, van snelweg naar strand, met genoeg luxe spacing om alles te laten ademen.</p></article></div><div className="dashGrid"><Info icon={<CalendarDays/>} label="Dagen tot vertrek" value={daysLeft}/><Info icon={<Users/>} label="Reizigers" value="16"/><Info icon={<Tent/>} label="Campingdagen" value="19"/><Info icon={<Sun/>} label="Fun index" value="12/10"/></div><div className="feed"><h2>Live Intelligence Feed</h2>{['Vakantiepres gedetecteerd bij meerdere reizigers','Kofferbakruimte alvast als onvoldoende beoordeeld','Zonnebrandprotocol staat op scherp','Gelato-radar warmt langzaam op','Groepsappactiviteit verwacht: extreem'].map((x,i)=><motion.div initial={{opacity:0,x:-10}} animate={{opacity:1,x:0}} transition={{delay:i*.12}} className="feedItem" key={x}><BadgeAlert size={16}/>{x}</motion.div>)}</div></section>}
